...
exports.index = async (req, res) => {
  ...
  const processFilename = path.join(rootDir, 'exhibitions', tms, 'process', subFolder, `${id}.json`)
  const processedFilename = path.join(rootDir, 'exhibitions', tms, 'processed', subFolder, `${id}.json`)
  ...
  const query = queries.get('exhibition', searchFilter)
  ...
  return res.render('search/exhibition', req.templateValues)
}
